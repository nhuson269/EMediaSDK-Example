...
class VideoViewController: UIViewController {
    ...
    let client: EMediaClient = EMediaClient.init()
    var clientUrl: String!
    var clientStreamId: String!
    var clientToken: String!
    var clientMode: EMediaClientMode!
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if self.client.getCurrentMode() == EMediaClientMode.join {
        ...
        } else if self.client.getCurrentMode() == EMediaClientMode.publish {
        ...
        } else if self.client.getCurrentMode() == EMediaClientMode.play {
            self.fullVideoView.isHidden = false
            self.pipVideoView.isHidden = false
...
extension VideoViewController: EMediaClientDelegate {
    
    func clientDidConnect(_ client: EMediaClient) {
    ...
    func dataReceivedFromDataChannel(streamId: String, data: Data, binary: Bool) {
        EMediaClient.printf("Data is received from stream: \(streamId) is binary:\(binary) content: " + String(decoding: data, as: UTF8.self))
    ...
    func streamInformation(streamInfo: [StreamInformation]) {
        EMediaClient.printf("Incoming stream infos")
        for result in streamInfo {
            EMediaClient.printf("resolution width:\(result.streamWidth) heigh:\(result.streamHeight) video " + "bitrate:\(result.videoBitrate) audio bitrate:\(result.audioBitrate) codec:\(result.videoCodec)");
        }
    }
